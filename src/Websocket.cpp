...
void handleWebSocketData(uint8_t *data, size_t len)
{
  ...
  { // Text frame
    ...
    { // Confirm if the intended recipient
      if (command == "DISPLAY")
      {
        ...
        else if (value == "7")
        {
          // Custom Text
          int fourthColon = message.indexOf(':', thirdColon + 1);
          String customText = message.substring(thirdColon + 1, fourthColon);
          // Clear the array
          for (int i = 0; i < MAX_TEXT_LINES; i++)
          {
            arrText[i] = "";
          }
          arrText[0] = customText;          
          // // Populate array with each character from customText
          // int textLen = min((int)customText.length(), MAX_TEXT_LINES);
          // for (int i = 0; i < textLen; i++)
          // {
          //   arrText[i] = String(customText[i]); // Convert each char to String
          // }
          displayImage(7);
        }
        else
      ...
      else if (command == "RPM")
      {
        ...
        {
          RPM_delay = 60000000; // Set to a very high value if RPM is zero or invalid
        }
